...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v96' not in st.session_state: st.session_state.sel_v96 = False
        if t3.button("✅ Select All"): st.session_state.sel_v96 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v96 = False; st.rerun()
        ...
        df_display.insert(0, "Select", st.session_state.sel_v96)
        
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v96")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v96")
...
# --- 5. Company Register (【1:1 三圖次序還原】) ---
    ...
    with c1:
        ...
        name_en = st.text_input("Eng", value=d['en'], label_visibility="collapsed")
    with c2:
        st.markdown("⚠️ Company Chinese Name :red[(Required!)]")
        name_ch = st.text_input("Chi", value=d['ch'], label_visibility="collapsed")
    ...
    # --- 第二部分: Address & Contact + Seal Storage (image_1e0cd9.png) ---
    ...
    with ca2:
        ...
        corres_addr = st.text_area("Cor", value=d['ca'], label_visibility="collapsed")
    ...
    # --- 第三部分: Compliance Filings (image_1e0d1a.png / 置底) ---
    # ND2A
    st.subheader("📑 Company Secretary Appointment (ND2A)")
    cc1, cc2, cc3, cc4 = st.columns([3, 3, 3, 1])
    with cc1:
        n2e = st.date_input("Effective Date (Appt)", value=d['n2e'], key="n2e_v96")
    with cc2:
        n2f = st.date_input("Filing Date (ND2A)", value=d['n2f'], key="n2f_v96")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e:
            n2_dl = (n2e + timedelta(days=15))
            st.markdown(f"**Deadline: :red[{n2_dl}]**") # 紅色顯示實際日期
    with cc4:
        n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v96")
    
    # ND4
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5:
        n4e = st.date_input("Effective Date (Resign)", value=d['n4e'], key="n4e_v96")
    with cc6:
        n4f = st.date_input("Filing Date (ND4)", value=d['n4f'], key="n4f_v96")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e:
            n4_dl = (n4e + timedelta(days=15))
            st.markdown(f"**Deadline: :red[{n4_dl}]**") # 紅色顯示實際日期
    with cc8:
        n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v96")
    st.write("---")

    # --- 按鈕區 (最底) ---
    row_v96 = {'client_group': client_group, 'name_en': name_en, 'name_ch': name_ch, 'incorp_date': inc_date, 'incorp_place': inc_place, 'incorp_place_others': place_others, 'ci_no': ci_no, 'br_no': br_no, 'co_type': co_type, 'reg_addr': reg_addr, 'corres_addr': corres_addr, 'round_loc': round_l, 'sign_loc': sign_l, 'seal_loc': common_l, 'nd2a_eff_date': n2e, 'nd2a_file_date': n2f, 'nd2a_download': n2d, 'nd4_eff_date': n4e, 'nd4_file_date': n4f, 'nd4_download': n4d, 'dissolution_date': dis_date}

    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud"): pd.DataFrame([row_v96]).to_sql('companies', engine, if_exists='append', index=False); st.rerun()
    else:
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm"):
                ...
                pd.DataFrame([row_v96]).to_sql('companies', engine, if_exists='append', index=False); st.rerun()
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v96")
