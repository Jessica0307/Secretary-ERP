...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v93' not in st.session_state: st.session_state.sel_v93 = False
        if t3.button("✅ Select All"): st.session_state.sel_v93 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v93 = False; st.rerun()
        ...
        df_display.insert(0, "Select", st.session_state.sel_v93)
        
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v93")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v93")
...
# --- 5. Company Register (【嚴格 1:1 視圖還原】) ---
    ...
    client_group = st.selectbox("⚠️ Select Client Group (Required!)", [""] + groups, index=(groups.index(d['cg'])+1 if d['cg'] in groups else 0))
    
    c1, c2 = st.columns(2)
    name_en = c1.text_input("⚠️ Company English Name (Required!)", value=d['en'])
    name_ch = c2.text_input("⚠️ Company Chinese Name (Required!)", value=d['ch'])
    
    c3, c4 = st.columns(2)
    inc_date = c3.date_input("⚠️ Date of Incorporation (Required!)", value=d['idate'])
    inc_place = c4.selectbox("⚠️ Place of Incorporation (Required!)", ["", "HK", "BVI", "Others"], index=(["", "HK", "BVI", "Others"].index(d['place']) if d['place'] in ["", "HK", "BVI", "Others"] else 0))
    place_others = st.text_input("⚠️ Specify Others (Required!)", value=d['p_oth']) if inc_place == "Others" else ""
    
    st.write("---")
    c5, c6 = st.columns(2)
    ci_no = c5.text_input("⚠️ CI Number (Required!)", value=d['ci'])
    br_no = c6.text_input("⚠️ BR Number (Required!)", value=d['br'])
    co_type = st.selectbox("⚠️ Company Type (Required!)", ["", "Private Company", "Public Company", "Guarantee"], index=(["", "Private Company", "Public Company", "Guarantee"].index(d['type']) if d['type'] in ["", "Private Company", "Public Company", "Guarantee"] else 0))
    st.write("---")

    # --- Section 2: Address & Contact (image_1e0cd9.png) ---
    st.subheader("📍 Address & Contact")
    ca1, ca2 = st.columns(2)
    reg_addr = ca1.text_area("⚠️ Registered Office Address (Required!)", value=d['ra'])
    corres_addr = ca2.text_area("⚠️ Correspondence Address (Required!)", value=d['ca'])

    # --- Section 3: Seal Storage (image_1e0cd9.png) ---
    st.subheader("📔 Seal Storage")
    l1, l2, l3 = st.columns(3)
    round_l = l1.text_input("⚠️ Round Chop Location (Required!)", value=d['rl'])
    sign_l = l2.text_input("⚠️ Signature Chop Location (Required!)", value=d['sl'])
    common_l = l3.text_input("⚠️ Common Seal Location (Required!)", value=d['cl'])
    st.write("---")
    
    dis_date = st.date_input("Company Dissolution Date", value=d['dis'])
    st.write("---")

    # --- Section 4: Compliance Filings (image_1e0d1a.png / 置底 + 紅色實際 Deadline) ---
    # ND2A
    st.subheader("📑 Company Secretary Appointment (ND2A)")
    cc1, cc2, cc3, cc4 = st.columns([3, 3, 3, 1])
    n2e = cc1.date_input("Effective Date (Appt)", value=d['n2e'], key="n2e_v93")
    n2f = cc2.date_input("Filing Date (ND2A)", value=d['n2f'], key="n2f_v93")
    n2_dl = (n2e + timedelta(days=15)) if n2e else ''
    cc3.info("Statutory Period: 15 days")
    if n2e: cc3.markdown(f"**Deadline: :red[{n2_dl}]**") # 紅色顯示實際日期
    n2d = cc4.checkbox("Downloaded", value=d['n2d'], key="n2d_v93")
    
    # ND4
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    n4e = cc5.date_input("Effective Date (Resign)", value=d['n4e'], key="n4e_v93")
    n4f = cc6.date_input("Filing Date (ND4)", value=d['n4f'], key="n4f_v93")
    n4_dl = (n4e + timedelta(days=15)) if n4e else ''
    cc7.info("Statutory Period: 15 days")
    if n4e: cc7.markdown(f"**Deadline: :red[{n4_dl}]**") # 紅色顯示實際日期
    n4d = cc8.checkbox("Downloaded", value=d['n4d'], key="n4d_v93")
    st.write("---")

    # 按鈕邏輯 (image_1e0cd9.png)
    row_v93 = {'client_group': client_group, 'name_en': name_en, 'name_ch': name_ch, 'incorp_date': inc_date, 'incorp_place': inc_place, 'incorp_place_others': place_others, 'ci_no': ci_no, 'br_no': br_no, 'co_type': co_type, 'reg_addr': reg_addr, 'corres_addr': corres_addr, 'round_loc': round_l, 'sign_loc': sign_l, 'seal_loc': common_l, 'nd2a_eff_date': n2e, 'nd2a_file_date': n2f, 'nd2a_download': n2d, 'nd4_eff_date': n4e, 'nd4_file_date': n4f, 'nd4_download': n4d, 'dissolution_date': dis_date}

    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud"): pd.DataFrame([row_v93]).to_sql('companies', engine, if_exists='append', index=False); st.rerun()
    else:
        u, dl = st.columns(2)
        with u.popover("🆙 Update"):
            if st.button("Confirm Update"):
                df_all[df_all['name_en'] != target_name].to_sql('companies', engine, if_exists='replace', index=False)
                pd.DataFrame([row_v93]).to_sql('companies', engine, if_exists='append', index=False); st.rerun()
        with dl.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v93")
