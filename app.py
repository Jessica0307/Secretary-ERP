...
def generate_custom_pdf(selected_df):
    ...
    for _, row in selected_df.iterrows():
        ...
        if place == 'HK':
            ...
            dynamic_annual_rows += f"<tr><th>BR Paid By / 負責方</th><td>{str(row.get('br_paid_by', 'Firm'))}</td></tr>"
            dynamic_annual_rows += f"<tr><th>Last BR Resolved Date</th><td>{fmt_date(row.get('last_br_resolved_date'))}</td></tr>"
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        for _, row in df_raw.iterrows():
            ...
            this_anniv = get_anniv(today.year, base_date.month, base_date.day)
            
            # BR 循環計算
            active_br_anniv = this_anniv if today >= this_anniv - timedelta(days=30) else get_anniv(today.year - 1, base_date.month, base_date.day)
            if last_br is None or last_br < active_br_anniv:
                days_diff = (active_br_anniv - today).days
                resp_str = f" (負責方: {br_by})"
                if days_diff < 0: alerts["red"].append(f"🛑 **{name}** BR Fee Overdue (Due: {active_br_anniv.strftime('%Y/%m/%d')}){resp_str}")
                elif 0 <= days_diff <= 30: alerts["yellow"].append(f"⏳ **{name}** BR Fee Due Soon (Due: {active_br_anniv.strftime('%Y/%m/%d')}){resp_str}")
                    
            # AR 循環計算
            active_ar_anniv = this_anniv if today >= this_anniv else get_anniv(today.year - 1, base_date.month, base_date.day)
            active_ar_deadline = active_ar_anniv + timedelta(days=42)
            if last_ar is None or last_ar < active_ar_anniv:
                days_diff = (active_ar_deadline - today).days
                if days_diff < 0: alerts["red"].append(f"🛑 **{name}** AR Overdue (Due: {active_ar_deadline.strftime('%Y/%m/%d')})")
                elif 0 <= days_diff <= 30: alerts["yellow"].append(f"⏳ **{name}** AR Due Soon (Due: {active_ar_deadline.strftime('%Y/%m/%d')})")

        if alerts["red"] or alerts["yellow"]:
            with st.expander("🚨 Compliance Alerts (Action Required) 點擊展開", expanded=True):
                for msg in alerts["red"]: st.error(msg)
                for msg in alerts["yellow"]: st.warning(msg)
        # ==============================================================================
        ...
        if 'sel_v138' not in st.session_state: st.session_state.sel_v138 = False
        if t3.button("✅ Select All"): st.session_state.sel_v138 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v138 = False; st.rerun()
        
        existing_cols = [c for c in TEMPLATE_COLS if c in df_filtered.columns]
        df_display = df_filtered[existing_cols].copy()
        df_display.insert(0, "Select", st.session_state.sel_v138)
        
        total_count = len(df_filtered)
        st.markdown(f"📈 Total: **{total_count}** companies in current view.")

        # ==================== V138: 基礎欄位鎖定，解鎖常規申報進行批量直接編輯 ====================
        ...
        edit_df = st.data_editor(
            ...
            column_config={
                ...
                "br_paid_by": st.column_config.SelectboxColumn("BR Paid By", options=["Firm", "Client"], required=True),
                "last_br_resolved_date": st.column_config.DateColumn("BR Paid Date"),
                "last_ar_filed_date": st.column_config.DateColumn("AR Filed Date")
            }, 
        ...
        # 批量儲存按鈕
        if st.button("💾 Save Table Changes (批量儲存表格修改)", key="btn_save_grid_v138"):
            try:
                with engine.begin() as conn:
                    for _, r in edit_df.iterrows():
                        ...
                        br_dt_str = f"'{br_dt}'" if pd.notna(br_dt) and br_dt else "NULL"
                        ar_dt_str = f"'{ar_dt}'" if pd.notna(ar_dt) and ar_dt else "NULL"
                        
                        sql = f"""
                            UPDATE companies 
                            SET br_paid_by = '{br_by}', 
                                last_br_resolved_date = {br_dt_str}, 
                                last_ar_filed_date = {ar_dt_str} 
                            WHERE name_en = '{name_en_val}'
                        """
                        conn.execute(sql)
        ...
        if selected_count > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v138")
                if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v138"):
...
# --- 6. Company Register ---
    ...
    if inc_place == "HK":
        ...
        if is_hk_reg:
            ...
            with hk1: st.markdown("⚠️ HK Incorp. Date (YYYY/MM/DD) :red[(Required!)]"); hk_idate = st.date_input("HK_Date", value=to_date(hk_idate), min_value=MIN_DATE, label_visibility="collapsed")
    ...
    # ==================== 📅 V138 年度常規申報 (特大醒目色框 + 負責方選擇) ====================
    if inc_place == "HK" or is_hk_reg:
        st.write("---"); st.header("📅 Annual Obligations / 年度常規申報")
        
        base = hk_idate if is_hk_reg else inc_date
        if base:
            today_cal = datetime.now().date()
            anniv = get_anniv(today_cal.year, base.month, base.day)
            nxt_br = anniv if today_cal >= anniv - timedelta(days=30) else get_anniv(today_cal.year - 1, base.month, base.day)
            ...
            with col_m1:
                if br_days < 0:
                    st.error(f"### 🚨 BR Fee 繳費死線：**{nxt_br.strftime('%Y/%m/%d')}**\n\n⚠️ **已經逾期 {abs(br_days)} 天！**")
                elif br_days <= 30:
                    st.warning(f"### ⏳ BR Fee 繳費死線：**{nxt_br.strftime('%Y/%m/%d')}**\n\n⏰ **即將到期（剩餘 {br_days} 天）**")
                else:
                    st.success(f"### 🟢 BR Fee 繳費死線：**{nxt_br.strftime('%Y/%m/%d')}**\n\n✅ 運作正常（剩餘 {br_days} 天）")
                    
            with col_m2:
                if ar_days < 0:
                    st.error(f"### 🚨 AR 周年申報死線：**{nxt_ar.strftime('%Y/%m/%d')}**\n\n⚠️ **已經逾期 {abs(ar_days)} 天！**")
                elif ar_days <= 30:
                    st.warning(f"### ⏳ AR 周年申報死線：**{nxt_ar.strftime('%Y/%m/%d')}**\n\n⏰ **即將到期（剩餘 {ar_days} 天）**")
                else:
                    st.success(f"### 🟢 AR 周年申報死線：**{nxt_ar.strftime('%Y/%m/%d')}**\n\n✅ 運作正常（剩餘 {ar_days} 天）")
        
        o1, o2, o3 = st.columns([3, 4, 4])
        with o1:
            st.markdown("BR Fee 負責方")
            br_paid_by_opts = ["Firm (我哋交)", "Client (客自己交)"]
            br_paid_by_idx = 1 if d.get('br_paid_by') == "Client" else 0
            br_paid_by_sel = st.selectbox("BR_Paid_By_Sel", br_paid_by_opts, index=br_paid_by_idx, label_visibility="collapsed")
            br_paid_by = "Client" if "Client" in br_paid_by_sel else "Firm"
        with o2: 
            st.markdown("BR Paid Date / 商業登記費最新處理日期")
            l_br = st.date_input("L_BR", value=to_date(d['l_br']), min_value=MIN_DATE, label_visibility="collapsed")
            st.caption("💡 若由客自行繳交，選取負責方為 Client 並填入確認日期即可。")
        with o3: 
            st.markdown("AR Filed Date / 周年申報表最新提交日期")
    ...
    # ==================== 📝 100% 完美鎖定 V131 個性化 Compliance Filings ====================
    st.write("---"); st.header("📝 Compliance Filings")
    st.subheader("📑 Company Secretary Appointment (ND2A)")
    cc1, cc2, cc3, cc4 = st.columns([3, 3, 3, 1])
    with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v138")
    with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v138")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e: n2_deadline = (n2e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
    with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v138")
    
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v138")
    with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v138")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
    with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v138")
    # =======================================================================================
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v138"):
            if missing: st.error(f"❌ Missing: {', '.join(missing)}")
            else:
                try:
                    pd.DataFrame([row_v138]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v138"):
                if missing: st.error(f"❌ Missing: {', '.join(missing)}")
                else:
                    try:
                        ...
                        st.error(f"🛑 SQL Error Detected! Rollback completed. Your data is SAFE. Details: {trans_err}")
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v138")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v138"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v138")
    if st.button("Add Group", key="btn_add_group_v138"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        g_df = g_df.sort_values(by=['group_name'], na_position='last')
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v138")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v138")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v138")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v138"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v138"): 
                g_df[g_df['group_name'] != target].to_sql('client_groups', engine, if_exists='replace', index=False); st.rerun()

# --- 8. Data Exchange (V138 Excel 防線 + 新增欄位兼容) ---
    ...
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v138")
    st.write("---")
    
    up = st.file_uploader("Upload XLSX to Review Changes", type=["xlsx"], key="file_uploader_v138")
    if up:
        try:
            ...
            for idx, row_new in up_df.iterrows():
                ...
                if missing_fields:
                    validation_errors.append(f"**Row {excel_row} ({name_en})** 漏咗填: :red[{', '.join(missing_fields)}]")

            if validation_errors:
                st.error("🛑 **上傳失敗：Excel 檔案內有必填項目漏空！請修正後再重新上傳。**")
                ...
                if diff_list:
                    ...
                    if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v138"):
                        ...
                        # 若 Excel br_paid_by 留空，預設填入 Firm
