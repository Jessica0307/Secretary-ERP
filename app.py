...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False), "incorp_date": st.column_config.DateColumn("Incorp Date", format="YYYY/MM/DD"), "nd2a_eff_date": st.column_config.DateColumn("ND2A Eff", format="YYYY/MM/DD"), "nd4_eff_date": st.column_config.DateColumn("ND4 Eff", format="YYYY/MM/DD")}, disabled=[c for c in df_display.columns if c != "Select"], hide_index=True, use_container_width=True, key="dashboard_editor_v44")
...
# --- 6. Data Exchange (恢復完整 V42 備份與模板功能) ---
