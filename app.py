...
# --- 3. Navigation ---
st.set_page_config(page_title="ERP Cloud V139", layout="wide")
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        # ==================== V139 Smart Alerts (Current Year strictly) ====================
        ...
        for _, row in df_raw.iterrows():
            ...
            # Focus strictly on the current calendar year (2026+)
            ...
            if br_by != 'Client':
                if not last_br or last_br.year < current_year:
                    ...
                    if days_diff < 0: alerts["red"].append(f"🛑 **{name}** BR Fee Overdue (Deadline: {br_dl.strftime('%Y/%m/%d')})")
                    elif 0 <= days_diff <= 30: alerts["yellow"].append(f"⏳ **{name}** BR Fee Due Soon (Deadline: {br_dl.strftime('%Y/%m/%d')})")
                    
            # AR Alert Logic
            if not last_ar or last_ar.year < current_year:
                days_diff = (ar_dl - today).days
                if days_diff < 0: alerts["red"].append(f"🛑 **{name}** AR Overdue (Deadline: {ar_dl.strftime('%Y/%m/%d')})")
                elif 0 <= days_diff <= 30: alerts["yellow"].append(f"⏳ **{name}** AR Due Soon (Deadline: {ar_dl.strftime('%Y/%m/%d')})")

        if alerts["red"] or alerts["yellow"]:
            with st.expander("🚨 Compliance Alerts (Action Required)", expanded=True):
        ...
        if 'sel_v139' not in st.session_state: st.session_state.sel_v139 = False
        if t3.button("✅ Select All"): st.session_state.sel_v139 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v139 = False; st.rerun()
        
        existing_cols = [c for c in TEMPLATE_COLS if c in df_filtered.columns]
        df_display = df_filtered[existing_cols].copy()
        df_display.insert(0, "Select", st.session_state.sel_v139)
        
        total_count = len(df_filtered)
        st.markdown(f"📈 Total: **{total_count}** companies in current view.")

        # ==================== V139: Interactive Batch Editor ====================
        st.info("💡 **Batch Editor:** Double-click the cells below to directly modify compliance status. Click **Save Batch Edits** to apply.")
        ...
        if st.button("💾 Save Batch Edits", key="btn_save_grid_v139"):
            try:
                with engine.begin() as conn:
                    for _, r in edit_df.iterrows():
                        ...
                        if br_by == 'Client':
                            br_dt_str = "NULL" # Client pays, so date is strictly nullified
                        else:
        ...
        if selected_count > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v139")
                if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v139"):
...
# --- 6. Company Register ---
    ...
    if inc_place == "HK" or is_hk_reg:
        ...
        if base:
            ...
            # Purely current year calculations
            ...
            with col_m1:
                if d.get('br_paid_by') == "Client":
                    ...
                    if br_days < 0:
                        st.error(f"### 🚨 BR Fee Deadline: **{nxt_br.strftime('%Y/%m/%d')}**\n\n⚠️ **Overdue by {abs(br_days)} days**")
                    elif br_days <= 30:
                        st.warning(f"### ⏳ BR Fee Deadline: **{nxt_br.strftime('%Y/%m/%d')}**\n\n⏰ **Due in {br_days} days**")
                    else:
                        st.success(f"### 🟢 BR Fee Deadline: **{nxt_br.strftime('%Y/%m/%d')}**\n\n✅ **Status: Normal**")
                        
            with col_m2:
                if ar_days < 0:
                    st.error(f"### 🚨 AR Deadline: **{nxt_ar.strftime('%Y/%m/%d')}**\n\n⚠️ **Overdue by {abs(ar_days)} days**")
                elif ar_days <= 30:
                    st.warning(f"### ⏳ AR Deadline: **{nxt_ar.strftime('%Y/%m/%d')}**\n\n⏰ **Due in {ar_days} days**")
                else:
                    st.success(f"### 🟢 AR Deadline: **{nxt_ar.strftime('%Y/%m/%d')}**\n\n✅ **Status: Normal**")
    ...
    with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v139")
    with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v139")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e: n2_deadline = (n2e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
    with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v139")
    
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v139")
    with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v139")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
    with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v139")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v139"):
            if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
            else:
                try:
                    pd.DataFrame([row_v139]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v139"):
                if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                else:
                    try:
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v139")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v139"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v139")
    if st.button("Add Group", key="btn_add_group_v139"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        g_df = g_df.sort_values(by=['group_name'], na_position='last')
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v139")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v139")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v139")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v139"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v139"): 
...
# --- 8. Data Exchange ---
    ...
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v139")
    st.write("---")
    
    up = st.file_uploader("Upload XLSX to Review Changes", type=["xlsx"], key="file_uploader_v139")
    if up:
        try:
            ...
            if validation_errors:
                ...
                if diff_list:
                    ...
                    if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v139"):
                        ...
                        # Apply nullification logic for Client paid BRs in batch upload
