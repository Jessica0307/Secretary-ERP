...
# --- 2. Navigation ---
st.set_page_config(page_title="ERP Cloud V41", layout="wide")
...
# --- PDF 生成函式 (修正中文 + 每頁標題 + 一公司一頁 + 逐項分開 + 格式化日期) ---
def generate_custom_pdf(selected_df):
    ...
    for _, row in selected_df.iterrows():
        ...
        <div class="company-card">
            ...
            <div class="section-title">Items Storage / 物品存放</div>
            <table>
...
if choice == "📤 Data Exchange":
    ...
    col_ex1.download_button(label="📥 Download Template", data=buffer_tmp.getvalue(), file_name="Template.xlsx")
    ...
    col_ex2.download_button(label="📦 Full Backup (Excel)", data=buffer_all.getvalue(), file_name=f"Backup_{datetime.now().strftime('%Y%m%d')}.xlsx")
...
# --- 4. Company Register (V32 鎖定) ---
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        ...
        with cb2.popover("🚨 DELETE"):
            confirm = st.text_input("Type DELETE", key="single_del")
            if st.button("🔥 DELETE NOW", disabled=(confirm != "DELETE")):
...
# --- 5. Dashboard ---
    ...
    if not df_raw.empty:
        ...
        if t4.button("🧹 Clear"): st.session_state.select_state = False; st.rerun()
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, disabled=[c for c in df_display.columns if c != "Select"], hide_index=True, use_container_width=True, key="dashboard_v41")
        
        selected_rows = edited_df[edited_df["Select"] == True]
        if len(selected_rows) > 0:
            st.info(f"Selected {len(selected_rows)} rows")
            ...
            with act2.popover("🧨 BATCH DELETE"):
                conf = st.text_input("Type DELETE", key="batch_del")
...
# --- 6. Group Management ---
    ...
    if not g_df.empty:
        ...
        with st.popover("🗑️ Delete Group"):
            conf = st.text_input("Type DELETE", key="g_del")
            if st.button("Delete Group", disabled=(conf != "DELETE")):
