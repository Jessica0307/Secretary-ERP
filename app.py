...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v111' not in st.session_state: st.session_state.sel_v111 = False
        if t3.button("✅ Select All"): st.session_state.sel_v111 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v111 = False; st.rerun()
        ...
        df_display.insert(0, "Select", st.session_state.sel_v111)
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v111")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v111")
                if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v111"):
                    df_raw[~df_raw["name_en"].isin(selected["name_en"].tolist())].to_sql('companies', engine, if_exists='replace', index=False); st.rerun()
    else: st.info("No records.")

# --- 6. Company Register (新增 Batch Update 模式) ---
elif choice == "🏢 Company Register":
    st.title("🏢 Company Records Management")
    mode = st.radio("Mode", ["🆕 Add New", "✏️ Edit Existing", "📋 Copy Existing", "📦 Batch Update"], horizontal=True)
    ...
    if mode == "📦 Batch Update":
        st.header("Batch Update Addresses & Seals")
        target_cos = st.multiselect("Select Companies to Update", df_all['name_en'].tolist())
        
        if target_cos:
            st.info(f"You have selected {len(target_cos)} companies.")
            st.subheader("📍 New Addresses")
            bc1, bc2 = st.columns(2)
            new_reg = bc1.text_area("New Registered Office Address")
            new_corres = bc2.text_area("New Correspondence Address")
            
            st.subheader("📔 New Seal Storage")
            bl1, bl2, bl3 = st.columns(3)
            new_round = bl1.text_input("New Round Chop Location")
            new_sign = bl2.text_input("New Signature Chop Location")
            new_common = bl3.text_input("New Common Seal Location")
            
            st.write("---")
            with st.popover("💾 Apply Batch Changes"):
                st.warning("This will overwrite the selected fields for ALL selected companies.")
                if st.button("Confirm Batch Update", key="btn_confirm_batch"):
                    updates = {}
                    if new_reg: updates['reg_addr'] = new_reg
                    if new_corres: updates['corres_addr'] = new_corres
                    if new_round: updates['round_loc'] = new_round
                    if new_sign: updates['sign_loc'] = new_sign
                    if new_common: updates['seal_loc'] = new_common
                    
                    if updates:
                        for field, val in updates.items():
                            df_all.loc[df_all['name_en'].isin(target_cos), field] = val
                        df_all.to_sql('companies', engine, if_exists='replace', index=False)
                        st.success(f"Successfully updated {len(target_cos)} companies!"); st.rerun()
                    else: st.error("Please fill in at least one field to update.")
        else: st.info("Please select at least one company from the list above.")

    # --- 其他原有模式 (Add/Edit/Copy) ---
    else:
        ...
        with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v111")
        with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v111")
        with cc3:
            st.info("Statutory Period: 15 days")
            if n2e: n2_deadline = (n2e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
        with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v111")
        st.subheader("📑 Company Secretary Resignation (ND4)")
        cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
        with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v111")
        with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v111")
        with cc7:
            st.info("Statutory Period: 15 days")
            if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
        with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v111")
        ...
        if mode in ["🆕 Add New", "📋 Copy Existing"]:
            if st.button("💾 Save To Cloud", key="btn_save_v111"):
                if missing: st.error(f"❌ Missing: {', '.join(missing)}")
                else: pd.DataFrame([row_v111]).to_sql('companies', engine, if_exists='append', index=False); st.success("✅ Success!"); st.rerun()
        else:
            u_col, d_col = st.columns(2)
            with u_col.popover("🆙 Update"):
                if st.button("Confirm Update", key="btn_update_v111"):
                    if missing: st.error(f"❌ Missing: {', '.join(missing)}")
                    else:
                        df_all[df_all['name_en'] != target_name].to_sql('companies', engine, if_exists='replace', index=False)
                        pd.DataFrame([row_v111]).to_sql('companies', engine, if_exists='append', index=False); st.success("✅ Updated!"); st.rerun()
            with d_col.popover("🚨 DELETE"):
                st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v111")
                if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v111"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v111")
    if st.button("Add Group", key="btn_add_group_v111"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v111")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v111")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v111")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v111"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v111"): 
...
# --- 8. Data Exchange (絕對鎖定 Export All) ---
    ...
    df_e = pd.read_sql("SELECT * FROM companies", engine); buf_e = io.BytesIO(); df_e.to_excel(buf_e, index=False)
    # 絕對鎖定：Export All
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v111")
    st.write("---")
    up = st.file_uploader("Upload XLSX File", type=["xlsx"], key="file_uploader_v111")
    if up and st.button("🚀 Confirm Upload", key="btn_bulk_upload_v111"):
        try:
            ...
            if invalid_rows: st.error("### 🛑 Upload Blocked"); [st.write(f"- {err}") for err in invalid_rows]; st.stop()
