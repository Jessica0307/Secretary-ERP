...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v118' not in st.session_state: st.session_state.sel_v118 = False
        if t3.button("✅ Select All"): st.session_state.sel_v118 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v118 = False; st.rerun()
        df_display = df_filtered.copy()
        df_display.insert(0, "Select", st.session_state.sel_v118)
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v118")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v118")
                if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v118"):
...
# --- 6. Company Register ---
    ...
    with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v118")
    with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v118")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e: n2_deadline = (n2e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
    with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v118")
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v118")
    with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v118")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
    with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v118")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v118"):
            if missing: st.error(f"❌ Missing: {', '.join(missing)}")
            else: pd.DataFrame([row_v118]).to_sql('companies', engine, if_exists='append', index=False); st.success("✅ Success!"); st.rerun()
    else:
        u_col, d_col = st.columns(2)
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v118"):
                if missing: st.error(f"❌ Missing: {', '.join(missing)}")
                else:
                    df_all[df_all['name_en'] != target_name].to_sql('companies', engine, if_exists='replace', index=False)
                    pd.DataFrame([row_v118]).to_sql('companies', engine, if_exists='append', index=False); st.success("✅ Updated!"); st.rerun()
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v118")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v118"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v118")
    if st.button("Add Group", key="btn_add_group_v118"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v118")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v118")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v118")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v118"):
...
# --- 8. Data Exchange (V118：對比表新增公司名顯示) ---
    ...
    # 鎖定 Export All
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v118")
    st.write("---")
    
    up = st.file_uploader("Upload XLSX to Review Changes", type=["xlsx"], key="file_uploader_v118")
    if up:
        try:
            ...
            if diff_list:
                ...
                if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v118"):
                ...
                st.info("No differences found. Database is up to date.")
