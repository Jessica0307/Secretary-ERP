...
def generate_custom_pdf(selected_df):
    ...
    # CSS 強化：確保 thead 每頁重複，company-card 每間公司強制分頁
    html_content = f"""
    <html>
    <head>
        <meta charset="UTF-8">
                ...
                <tr>
                    <td>
                        <div class="header-content">
                            <h1 style="margin:0;">Corporate Portfolio Report</h1>
    ...
    for _, row in selected_df.iterrows():
        ...
        <div class="company-card">
            ...
            <table class="data-table">
                <tr><th>Registered Office Address / 註冊地址</th><td>{row.get('reg_addr','')}</td></tr>
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v59")
        ...
        if len(selected_rows) > 0:
            ...
            with act1:
                if st.button("📥 Export Selected to PDF"):
                    ...
                    st.download_button(label="Download PDF Now", data=generate_custom_pdf(final_selected), file_name="Report.pdf", mime="application/pdf")
            with act2.popover("🧨 BATCH DELETE"):
                st.error("### 🛑 DANGER ZONE")
                conf_batch = st.text_input("Type **DELETE** to confirm", key="batch_del_v59")
...
# --- 5. Company Register (UI 鎖定截圖 image_2c2193.png + 紅框 + 雙刪除提醒) ---
    ...
    cc3.info("Statutory Period: 15 days"); n2d = cc4.checkbox("Downloaded", value=d['n2d'], key="n2d_v59")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        with st.popover("💾 Save To Cloud"):
            if st.button("Confirm Save"):
                row = {'client_group': client_group, 'name_en': name_en, 'name_ch': name_ch, 'incorp_date': inc_date, 'incorp_place': inc_place, 'incorp_place_others': place_others, 'ci_no': ci_no, 'br_no': br_no, 'co_type': d['type'], 'reg_addr': reg_addr, 'corres_addr': corres_addr, 'round_loc': round_l, 'sign_loc': sign_l, 'seal_loc': common_l, 'nd2a_eff_date': n2e, 'nd2a_file_date': n2f, 'nd2a_download': n2d, 'nd4_eff_date': d['n4e'], 'nd4_file_date': d['n4f'], 'nd4_download': d['n4d'], 'dissolution_date': d['dis']}
        ...
        with btn_d.popover("🚨 DELETE Record"):
            st.error(f"### 🛑 DANGER: Delete {target_name}?")
            conf_single = st.text_input("Type **DELETE** to confirm", key="single_del_v59")
            if st.button("🔥 Confirm Delete Now", disabled=(conf_single != "DELETE")):
                df_all[df_all['name_en'] != target_name].to_sql('companies', engine, if_exists='replace', index=False); st.rerun()

# --- 6. Group Management (強力提醒鎖定) ---
    ...
    if not g_df.empty:
        ...
        with c1.popover("✏️ Rename"):
            ren = st.text_input("New Name:"); conf_ren = st.text_input("Type **RENAME**")
            if st.button("Confirm", disabled=(conf_ren != "RENAME")):
        ...
        with c2.popover("🗑️ Delete Group"):
            st.error(f"### ⚠️ WARNING: Delete '{target}'?")
            conf_grp = st.text_input("Type **DELETE** to confirm", key="grp_del_v59")
            if st.button("🔥 Confirm Delete", disabled=(conf_grp != "DELETE")):
                g_df[g_df['group_name'] != target].to_sql('client_groups', engine, if_exists='replace', index=False); st.rerun()

# --- 7. Data Exchange (鎖定雙欄 + 回饋提示) ---
    ...
    if up:
        if st.button("🚀 Confirm Bulk Upload"):
            try:
                ...
                st.success("✅ Uploaded Successfully!") # 確保回饋
                st.balloons()
