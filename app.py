...
# --- 1. Database Connection (鎖定 V42 邏輯) ---
try:
    ...
    for _, row in selected_df.iterrows():
        ...
        <div class="company-card">
            ...
            <table>
                ...
                <tr><th>Round / Sign / Seal Location</th><td>{row.get('round_loc','')} / {row.get('sign_loc','')} / {row.get('seal_loc','')}</td></tr>
            </table>
        </div>"""
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False), "incorp_date": st.column_config.DateColumn("Incorp Date", format="YYYY/MM/DD"), "nd2a_eff_date": st.column_config.DateColumn("ND2A Eff", format="YYYY/MM/DD"), "nd4_eff_date": st.column_config.DateColumn("ND4 Eff", format="YYYY/MM/DD")}, disabled=[c for c in df_display.columns if c != "Select"], hide_index=True, use_container_width=True, key="dashboard_editor_v43")
...
# --- 4. Company Register (鎖定 V42) ---
    ...
    place_others = st.text_input("Specify Others", value=d['p_oth']) if inc_place == "Others" else ""
    st.write("---")
    col_ci, col_br = st.columns(2); ci_no = col_ci.text_input(red_label("CI", d['ci']), value=d['ci']); br_no = col_br.text_input(red_label("BR", d['br']), value=d['br'])
...
# --- 5. Group Management (新增改名與連動功能) ---
    ...
    if st.button("Add Group"):
        try:
            ...
            st.success(f"Group '{new_g}' added!"); st.rerun()
...
# --- 6. Data Exchange (鎖定 V42) ---
    ...
    st.download_button(label="📦 Export All Data (Backup)", data=buffer_all.getvalue(), file_name=f"Backup_{datetime.now().strftime('%Y%m%d')}.xlsx")
