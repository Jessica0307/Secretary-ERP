...
# --- 4. Company Register (加入強烈確認機制) ---
    ...
    if mode == "🆕 Add New":
        ...
        with b_col2.popover("🚨 DELETE RECORD"):
            ...
            st.error(f"YOU ARE ABOUT TO DELETE: \n\n**{name_en}**")
            st.write("This action CANNOT be undone. Are you absolutely sure?")
            if st.button("🔥 YES, DELETE FOREVER"):
                with engine.begin() as conn:
                    conn.execute(text("DELETE FROM companies WHERE id=:id"), {"id": target_id})
                st.warning("Company Deleted Successfully.")
                st.rerun()
