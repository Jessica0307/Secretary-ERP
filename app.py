...
# --- 3. Navigation ---
st.set_page_config(page_title="ERP Cloud V100", layout="wide")
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v100' not in st.session_state: st.session_state.sel_v100 = False
        if t3.button("✅ Select All"): st.session_state.sel_v100 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v100 = False; st.rerun()
        ...
        df_display.insert(0, "Select", st.session_state.sel_v100)
        
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v100")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v100")
...
# --- 6. Company Register (【V100 修正報錯 + 嚴格次序】) ---
    ...
    with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), key="n2e_v100")
    with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), key="n2f_v100")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e:
            n2_deadline = (n2e + timedelta(days=15))
            st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
    with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v100")
    
    # ND4
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), key="n4e_v100")
    with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), key="n4f_v100")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e:
            n4_deadline = (n4e + timedelta(days=15))
            st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
    with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v100")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud"):
            if missing: st.error(f"❌ Missing: {', '.join(missing)}")
                ...
                pd.DataFrame([row_v100]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v100")
