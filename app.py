...
# --- 2. Navigation (鎖定 V42) ---
st.set_page_config(page_title="ERP Cloud V45", layout="wide")
choice = st.sidebar.radio("Navigation", ["📊 Dashboard", "🏢 Company Register", "⚙️ Group Management", "📤 Data Exchange"])

# --- PDF 生成函式 (鎖定 V42：中文字型、YYYY/MM/DD 格式提示、逐項分行) ---
...
# --- 3. Dashboard (鎖定 V42：含篩選、全選、PDF 導出連動)[cite: 3] ---
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False), "incorp_date": st.column_config.DateColumn("Incorp Date", format="YYYY/MM/DD"), "nd2a_eff_date": st.column_config.DateColumn("ND2A Eff", format="YYYY/MM/DD"), "nd4_eff_date": st.column_config.DateColumn("ND4 Eff", format="YYYY/MM/DD")}, disabled=[c for c in df_display.columns if c != "Select"], hide_index=True, use_container_width=True, key="dashboard_editor_v45")
        ...
        if len(selected_rows) > 0:
            ...
            with act1:
                if st.button("📥 Export Selected to PDF"):
                    ...
                    st.download_button(label="Click to Download PDF", data=generate_custom_pdf(final_selected), file_name=f"Portfolio_Report_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf")
...
# --- 4. Company Register (鎖定 V42：含紅框必填、Edit/Copy/Delete 彈出窗)[cite: 3] ---
    ...
    groups = pd.read_sql("SELECT group_name FROM client_groups", engine)['group_name'].tolist()
    d = {'cg': "", 'en': "", 'ch': "", 'idate': None, 'place': "", 'p_oth': "", 'ci': "", 'br': "", 'type': "", 'ra': "", 'ca': "", 'rl': "", 'sl': "", 'cl': "", 'n2e': None, 'n2f': None, 'n4e': None, 'n4f': None}
    target_name = None
    if mode in ["✏️ Edit Existing", "📋 Copy Existing"] and not df_all.empty:
        ...
        if target_name != "":
            ...
            d = {'cg': row.get('client_group', ""), 'en': row.get('name_en', ""), 'ch': row.get('name_ch', ""), 'idate': row.get('incorp_date'), 'place': row.get('incorp_place', ""), 'p_oth': row.get('incorp_place_others', ""), 'ci': row.get('ci_no', ""), 'br': row.get('br_no', ""), 'type': row.get('co_type', ""), 'ra': row.get('reg_addr', ""), 'ca': row.get('corres_addr', ""), 'rl': row.get('round_loc', ""), 'sl': row.get('sign_loc', ""), 'cl': row.get('seal_loc', ""), 'n2e': row.get('nd2a_eff_date'), 'n2f': row.get('nd2a_file_date'), 'n4e': row.get('nd4_eff_date'), 'n4f': row.get('nd4_file_date')}
    ...
    c3, c4 = st.columns(2); inc_date = c3.date_input(red_label("Incorp Date", d['idate']), value=d['idate']); places = ["", "HK", "BVI", "Others"]; inc_place = c4.selectbox(red_label("Place", d['place']), places, index=(places.index(d['place']) if d['place'] in places else 0))
    place_others = st.text_input("Specify", value=d['p_oth']) if inc_place == "Others" else ""
    st.write("---")
    col_ci, col_br = st.columns(2); ci_no = col_ci.text_input(red_label("CI", d['ci']), value=d['ci']); br_no = col_br.text_input(red_label("BR", d['br']), value=d['br']); types = ["", "Private Company", "Public Company", "Company Limited by Guarantee"]; co_type = st.selectbox(red_label("Type", d['type']), types, index=(types.index(d['type']) if d['type'] in types else 0))
...
# --- 5. Group Management (鎖定 V42 + 植入連動改名功能)[cite: 3] ---
    ...
    if not g_df.empty:
        ...
        with col_g1.popover("✏️ Rename Group"):
            rename_to = st.text_input(f"Rename **{target_g}** to:"); conf_re = st.text_input("Type RENAME to confirm")
        ...
        with col_g2.popover("🗑️ Delete Group"):
            st.error(f"Delete **{target_g}**?"); conf_del = st.text_input("Type DELETE to confirm")
            if st.button("Confirm Delete Group", disabled=(conf_del != "DELETE")):
                g_df[g_df['group_name'] != target_g].to_sql('client_groups', engine, if_exists='replace', index=False); st.rerun()

# --- 6. Data Exchange (鎖定 V42：恢復完整備份、模板、上傳功能)[cite: 3] ---
