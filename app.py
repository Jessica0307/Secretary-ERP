...
# --- 1. Database Connection (鎖定 V42 穩定版) ---
try:
    ...
    for _, row in selected_df.iterrows():
        ...
        <div class="company-card">
            ...
            <table>
                ...
                <tr><th>Incorp. Place</th><td>{row.get('incorp_place','')} {f"({row.get('incorp_place_others','')})" if row.get('incorp_place') == 'Others' else ''}</td></tr>
                <tr><th>CI No. / BR No.</th><td>{row.get('ci_no','')} / {row.get('br_no','')}</td></tr>
            </table>
            <div class="section-title">Compliance Filings (YYYY/MM/DD)</div>
            <table>
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dashboard_editor")
        selected_rows = edited_df[edited_df["Select"] == True]
        if len(selected_rows) > 0:
            if st.button("📥 Export Selected to PDF"):
                final_selected = df_raw[df_raw['name_en'].isin(selected_rows['name_en'])]
                st.download_button(label="Download", data=generate_custom_pdf(final_selected), file_name="Portfolio.pdf", mime="application/pdf")
