...
# --- 2. Navigation ---
st.set_page_config(page_title="ERP Cloud V82", layout="wide")
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v82' not in st.session_state: st.session_state.sel_v82 = False
        if t3.button("✅ Select All"): st.session_state.sel_v82 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v82 = False; st.rerun()
        ...
        df_display.insert(0, "Select", st.session_state.sel_v82)
        
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v82")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE to confirm", key="batch_del_v82")
...
# --- 5. Company Register (介面 1:1 還原 + 三頁同步 + 法定申報置底) ---
    ...
    n2e = cc1.date_input("ND2A Effective Date (YYYY/MM/DD)", value=d['n2e'], key="n2e_v82")
    n2f = cc2.date_input("Filing Date", value=d['n2f'], key="n2f_v82")
    n2_dl = (n2e + timedelta(days=15)) if n2e else ''
    cc3.info(f"Statutory Period: 15 days\n\n**Deadline: :red[{n2_dl}]**")
    n2d = cc4.checkbox("Downloaded", value=d['n2d'], key="n2d_v82")
    
    # ND4
    st.write("**📝 Company Secretary Resignation (ND4)**")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    n4e = cc5.date_input("ND4 Effective Date (YYYY/MM/DD)", value=d['n4e'], key="n4e_v82")
    n4f = cc6.date_input("Filing Date", value=d['n4f'], key="n4f_v82")
    n4_dl = (n4e + timedelta(days=15)) if n4e else ''
    cc7.info(f"Statutory Period: 15 days\n\n**Deadline: :red[{n4_dl}]**")
    n4d = cc8.checkbox("Downloaded", value=d['n4d'], key="n4d_v82")
    st.write("---")

    # 按鈕邏輯
    row_v82 = {'client_group': client_group, 'name_en': name_en, 'name_ch': name_ch, 'incorp_date': inc_date, 'incorp_place': inc_place, 'incorp_place_others': place_others, 'ci_no': ci_no, 'br_no': br_no, 'co_type': co_type, 'reg_addr': reg_addr, 'corres_addr': corres_addr, 'round_loc': round_l, 'sign_loc': sign_l, 'seal_loc': common_l, 'nd2a_eff_date': n2e, 'nd2a_file_date': n2f, 'nd2a_download': n2d, 'nd4_eff_date': n4e, 'nd4_file_date': n4f, 'nd4_download': n4d, 'dissolution_date': dis_date}

    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save Record"): pd.DataFrame([row_v82]).to_sql('companies', engine, if_exists='append', index=False); st.rerun()
    else:
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update"):
                ...
                pd.DataFrame([row_v82]).to_sql('companies', engine, if_exists='append', index=False); st.rerun()
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v82")
