...
# 💡 V173: Added +2 to provide a 1-year buffer so the user can process next year's compliance/invoices ahead of time
active_years = list(range(2025, current_system_year + 2))

# --- 3. Navigation ---
st.set_page_config(page_title="ERP Cloud V173", layout="wide")
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        with tab1:
            ...
            if 'sel_v173' not in st.session_state: st.session_state.sel_v173 = False
            if t3.button("✅ Select All"): st.session_state.sel_v173 = True; st.rerun()
            if t4.button("🧹 Clear All"): st.session_state.sel_v173 = False; st.rerun()
            ...
            df_display.insert(1, "Select", st.session_state.sel_v173)
            ...
            if st.button("💾 Save Batch Edits", key="btn_save_grid_v173"):
                try:
            ...
            if len(selected) > 0:
                ...
                with c_act4.popover("🧨 BATCH DELETE"):
                    st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v173")
                    if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v173"):
        ...
        with tab2:
            ...
            if not df_alerts.empty:
                ...
                if 'sel_alert_v173' not in st.session_state: st.session_state.sel_alert_v173 = False
                if ta3.button("✅ Select All", key="sel_all_alert"): st.session_state.sel_alert_v173 = True; st.rerun()
                if ta4.button("🧹 Clear All", key="clr_all_alert"): st.session_state.sel_alert_v173 = False; st.rerun()
                ...
                df_alerts_display.insert(1, "Select", st.session_state.sel_alert_v173)
        ...
        with tab3:
            ...
            # 💡 Default is smart enough to stay on 'current year' despite the +1 buffer
            ...
            if 'sel_inv_v173' not in st.session_state: st.session_state.sel_inv_v173 = False
            if ti6.button("✅ Select All", key="sel_all_inv"): st.session_state.sel_inv_v173 = True; st.rerun()
            if ti7.button("🧹 Clear All", key="clr_all_inv"): st.session_state.sel_inv_v173 = False; st.rerun()
            ...
            if not df_inv.empty:
                ...
                df_inv_display.insert(1, "Select", st.session_state.sel_inv_v173)
...
# --- 6. Company Register ---
    ...
    if inc_place == "HK":
        ...
        with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v173", format="YYYY/MM/DD")
        with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v173", format="YYYY/MM/DD")
        with cc3:
        ...
        with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v173")
        
        st.subheader("📑 Company Secretary Resignation (ND4)")
        cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
        with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v173", format="YYYY/MM/DD")
        with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v173", format="YYYY/MM/DD")
        with cc7:
            st.info("Statutory Period: 15 days")
            if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline.strftime('%Y/%m/%d')}]**") 
        with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v173")
        ...
        with c_nn1: nn6_e = st.date_input("Effective Date", value=to_date(d['nn6_e']), min_value=MIN_DATE, key="nn6_e_v173", format="YYYY/MM/DD")
        with c_nn2: nn6_f = st.date_input("Filing Date (NN6)", value=to_date(d['nn6_f']), min_value=MIN_DATE, key="nn6_f_v173", format="YYYY/MM/DD")
        with c_nn3:
            st.info("Statutory Period: 1 Month")
            if nn6_e:
                nn6_deadline = add_one_month(nn6_e)
                st.markdown(f"**Deadline: :red[{nn6_deadline.strftime('%Y/%m/%d')}]**")
        with c_nn4: nn6_d = st.checkbox("Downloaded", value=d['nn6_d'], key="nn6_d_v173")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v173"):
            if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
            else:
                try:
                    pd.DataFrame([row_v173]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v173"):
                if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                else:
                    try:
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v173")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v173"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v173")
    if st.button("Add Group", key="btn_add_group_v173"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        g_df = g_df.sort_values(by=['group_name'], na_position='last')
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v173")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v173")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v173")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v173"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v173"): 
...
# --- 8. Data Exchange ---
    ...
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v173")
    st.write("---")
    
    up = st.file_uploader("Upload XLSX to Review Changes", type=["xlsx"], key="file_uploader_v173")
    if up:
        try:
            ...
            if validation_errors:
                ...
                if diff_list or True: 
                    ...
                    if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v173"):
