...
# --- 3. Navigation ---
st.set_page_config(page_title="ERP Cloud V174", layout="wide")
    ...
    for idx, row in df_export.iterrows():
        ...
        if isinstance(rec_dict, dict):
            ...
            for y in active_years:
                ...
                prev_br_by = br_by
                df_export.at[idx, f'{y} BR Paid By'] = br_by
...
def generate_inv_excel(df, year, month_disp, hide_client_group=False):
    ...
    for _, item in df.iterrows():
        ...
        for col_idx, h in enumerate(headers, 1):
            ...
            # Smart Color Badge Fill for Excel
            if h == "Billing Item":
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        # 💡 V174: First Pass - Populate df_raw with Smart Inheritance
        for index, row in df_raw.iterrows():
            ...
            for y in active_years:
                ...
                prev_br_by = br_by
                df_raw.at[index, f'{y}_br_paid_by'] = br_by
        ...
        for _, row in df_raw.iterrows():
            ...
            for y in active_years:
                ...
                # Directly read the accurately inherited values from df_raw
        ...
        with tab1:
            ...
            if 'sel_v174' not in st.session_state: st.session_state.sel_v174 = False
            if t3.button("✅ Select All"): st.session_state.sel_v174 = True; st.rerun()
            if t4.button("🧹 Clear All"): st.session_state.sel_v174 = False; st.rerun()
            ...
            df_display.insert(1, "Select", st.session_state.sel_v174)
            ...
            if st.button("💾 Save Batch Edits", key="btn_save_grid_v174"):
                try:
            ...
            if len(selected) > 0:
                ...
                with c_act4.popover("🧨 BATCH DELETE"):
                    st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v174")
                    if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v174"):
        ...
        with tab2:
            ...
            if not df_alerts.empty:
                ...
                if 'sel_alert_v174' not in st.session_state: st.session_state.sel_alert_v174 = False
                if ta3.button("✅ Select All", key="sel_all_alert"): st.session_state.sel_alert_v174 = True; st.rerun()
                if ta4.button("🧹 Clear All", key="clr_all_alert"): st.session_state.sel_alert_v174 = False; st.rerun()
                ...
                df_alerts_display.insert(1, "Select", st.session_state.sel_alert_v174)
        ...
        with tab3:
            ...
            if 'sel_inv_v174' not in st.session_state: st.session_state.sel_inv_v174 = False
            if ti6.button("✅ Select All", key="sel_all_inv"): st.session_state.sel_inv_v174 = True; st.rerun()
            if ti7.button("🧹 Clear All", key="clr_all_inv"): st.session_state.sel_inv_v174 = False; st.rerun()
            ...
            for index, row in df_raw.iterrows():
                ...
                # 💡 V174: Smart Inheritance for Invoicing Schedule
                prev_br_by = 'Firm'
                br_by = 'Firm'
                for y in active_years:
                    y_data = rec_dict.get(str(y), {})
                    if incorp_year and y < incorp_year:
                        br_by = 'N/A'
                    elif incorp_year and y == incorp_year:
                        br_by = y_data.get('br_paid_by', 'Firm')
            ...
            if not df_inv.empty:
                ...
                df_inv_display.insert(1, "Select", st.session_state.sel_inv_v174)
...
# --- 6. Company Register ---
    ...
    if inc_place == "HK" or is_hk_reg:
        ...
        if base:
            ...
            # 💡 V174: Smart Inheritance in Company Register UI
            prev_br_by = 'Firm'
            for y in active_years:
                with st.expander(f"📌 {y} 年度申報 (Year {y})", expanded=True):
                    ...
                    with o1:
                        ...
                        # 💡 Key update: whatever the user selects HERE becomes the fallback for the next year!
                        prev_br_by = br_by
    ...
    if inc_place == "HK":
        ...
        with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v174", format="YYYY/MM/DD")
        with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v174", format="YYYY/MM/DD")
        with cc3:
        ...
        with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v174")
        
        st.subheader("📑 Company Secretary Resignation (ND4)")
        cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
        with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v174", format="YYYY/MM/DD")
        with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v174", format="YYYY/MM/DD")
        with cc7:
            st.info("Statutory Period: 15 days")
            if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline.strftime('%Y/%m/%d')}]**") 
        with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v174")
        ...
        with c_nn1: nn6_e = st.date_input("Effective Date", value=to_date(d['nn6_e']), min_value=MIN_DATE, key="nn6_e_v174", format="YYYY/MM/DD")
        with c_nn2: nn6_f = st.date_input("Filing Date (NN6)", value=to_date(d['nn6_f']), min_value=MIN_DATE, key="nn6_f_v174", format="YYYY/MM/DD")
        with c_nn3:
            st.info("Statutory Period: 1 Month")
            if nn6_e:
                nn6_deadline = add_one_month(nn6_e)
                st.markdown(f"**Deadline: :red[{nn6_deadline.strftime('%Y/%m/%d')}]**")
        with c_nn4: nn6_d = st.checkbox("Downloaded", value=d['nn6_d'], key="nn6_d_v174")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v174"):
            if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
            else:
                try:
                    pd.DataFrame([row_v174]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v174"):
                if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                else:
                    try:
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v174")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v174"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v174")
    if st.button("Add Group", key="btn_add_group_v174"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        g_df = g_df.sort_values(by=['group_name'], na_position='last')
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v174")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v174")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v174")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v174"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v174"): 
...
# --- 8. Data Exchange ---
    ...
    for idx, row in df_export.iterrows():
        ...
        if isinstance(rec_dict, dict):
            # 💡 V174: Smart Inheritance for Excel Export
            prev_br_by = 'Firm'
            for y in active_years:
                if incorp_year and y < incorp_year:
                    df_export.at[idx, f'{y} BR Paid By'] = 'N/A'
    ...
    up = st.file_uploader("Upload XLSX to Review Changes", type=["xlsx"], key="file_uploader_v174")
    if up:
        try:
            ...
            if validation_errors:
                ...
                for _, row_new in up_df.iterrows():
                    ...
                    if not old_row.empty:
                        ...
                        # 💡 V174: Smart Inheritance for Data Import Sync
                        prev_br_by = 'Firm'
                        for y in active_years:
                            y_str = str(y)
                            y_data = rec_dict.get(y_str, {})
                ...
                if diff_list or True: 
                    ...
                    if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v174"):
