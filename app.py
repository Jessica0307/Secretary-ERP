        ...
        if len(selected_rows) > 0:
            ...
            with act1:
                if st.button("📥 Export Selected to PDF"):
                    ...
                    st.download_button(label="Download PDF", data=pdf_bytes, file_name=f"Portfolio_{datetime.now().strftime('%Y%m%d')}.pdf", mime="application/pdf")
...
# --- 6. Group Management ---
    ...
    if not g_df.empty:
        ...
        with st.popover("🗑️ Delete Group"):
            conf = st.text_input("Type DELETE", key="group_del_input")
...
# --- 7. Data Exchange ---
    ...
    col_ex2.download_button(label="📦 Export All Excel (Backup)", data=buffer_all.getvalue(), file_name=f"Backup_{datetime.now().strftime('%Y%m%d')}.xlsx")
    st.write("---")
