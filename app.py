...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        if 'sel_v117' not in st.session_state: st.session_state.sel_v117 = False
        if t3.button("✅ Select All"): st.session_state.sel_v117 = True; st.rerun()
        if t4.button("🧹 Clear All"): st.session_state.sel_v117 = False; st.rerun()
        
        df_display = df_filtered.copy()
        df_display.insert(0, "Select", st.session_state.sel_v117)
        
        edit_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v117")
        ...
        if len(selected) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v117")
                if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v117"):
...
# --- 6. Company Register ---
    ...
    with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v117")
    with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v117")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e: n2_deadline = (n2e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
    with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v117")
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v117")
    with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v117")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
    with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v117")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v117"):
            if missing: st.error(f"❌ Missing: {', '.join(missing)}")
            else: pd.DataFrame([row_v117]).to_sql('companies', engine, if_exists='append', index=False); st.success("✅ Success!"); st.rerun()
    else:
        u_col, d_col = st.columns(2)
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v117"):
                if missing: st.error(f"❌ Missing: {', '.join(missing)}")
                else:
                    df_all[df_all['name_en'] != target_name].to_sql('companies', engine, if_exists='replace', index=False)
                    pd.DataFrame([row_v117]).to_sql('companies', engine, if_exists='append', index=False); st.success("✅ Updated!"); st.rerun()
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v117")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v117"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v117")
    if st.button("Add Group", key="btn_add_group_v117"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v117")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v117")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v117")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v117"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v117"): 
                g_df[g_df['group_name'] != target].to_sql('client_groups', engine, if_exists='replace', index=False); st.rerun()

# --- 8. Data Exchange (V117：日期格式純化與 Export 對齊) ---
    ...
    # 【V117：Export 之前將所有日期強制轉為純日期字串，防止 Excel 加時間】
    df_db = pd.read_sql("SELECT * FROM companies", engine)
    df_export = df_db.copy()
    ...
    # 鎖定 Export All
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v117")
    st.write("---")
    
    up = st.file_uploader("Upload XLSX to Review Changes (Dates Standardized)", type=["xlsx"], key="file_uploader_v117")
    if up:
        try:
            up_df = pd.read_excel(up, engine='openpyxl', keep_default_na=False)
            existing_df = pd.read_sql("SELECT * FROM companies", engine)
            
            # 【V117 關鍵修復：上傳時強制純化日期格式】
            ...
            for col in ["incorp_date", "nd2a_eff_date", "nd2a_file_date", "nd4_eff_date", "nd4_file_date", "dissolution_date"]:
                if col in up_df.columns:
                    up_df[col] = pd.to_datetime(up_df[col], errors='coerce').dt.date
            ...
            for _, row_new in up_df.iterrows():
                ...
                if not old_row.empty:
                    ...
                    for col in cols_to_compare:
                        ...
                        # 對比時統一轉做 string 費事 format 唔同
                        if old_val != new_val:
                            diff_list.append({"BR Number": br_num, "Field": col, "Old Value": old_val, "New Value": new_val})
                else:
                    diff_list.append({"BR Number": br_num, "Field": "NEW RECORD", "Old Value": "N/A", "New Value": f"Company: {row_new.get('name_en')}"})

            if diff_list:
                st.subheader("🔍 Review Changes")
                st.table(pd.DataFrame(diff_list))
                if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v117"):
                    ...
                    st.success("✅ Sync Completed! Dates standardized."); st.balloons(); st.rerun()
            else:
                st.info("No differences found. Everything is already in sync.")
        except Exception as e: st.error(f"Error during preview: {e}")
