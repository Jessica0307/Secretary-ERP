...
# --- 3. Navigation ---
st.set_page_config(page_title="ERP Cloud V146", layout="wide")
                        ...
                        <div class="company-card">
                            <div class="section-group">
                                ...
                                <table class="info-table">
                                    <tr><th>Client Group</th><td>__CLIENT_GROUP__</td></tr>
                            ...
                            <div class="section-group">
                                ...
                                <table class="info-table">
                                    <tr><th>ND2A Eff. Date (YYYY/MM/DD)</th><td>__ND2A_EFF__</td></tr>
                                    <tr><th>ND4 Eff. Date (YYYY/MM/DD)</th><td>__ND4_EFF__</td></tr>
                                </table>
                            </div>
                        </div>
                    </td>
    ...
    for _, row in selected_df.iterrows():
        ...
        if place == 'HK' or is_hk_reg:
            ...
            for y in active_years:
                ...
                dynamic_annual_rows += f"<tr><th>BR Paid Date ({y})</th><td>{br_dt if br_dt else 'N/A'}</td></tr>"
                dynamic_annual_rows += f"<tr><th>AR Filed Date ({y})</th><td>{ar_dt if ar_dt else 'N/A'}</td></tr>"
    ...
    for idx, row in df_export.iterrows():
        ...
        if isinstance(rec_dict, dict):
            for y in active_years:
                ...
                df_export.at[idx, f'{y} BR Date'] = y_data.get('br_date', '')
                df_export.at[idx, f'{y} AR Date'] = y_data.get('ar_date', '')
                
    base_cols = [c for c in TEMPLATE_COLS if c in df_export.columns]
    dyn_cols = [f"{y} BR Paid By" for y in active_years] + [f"{y} BR Date" for y in active_years] + [f"{y} AR Date" for y in active_years]
    dyn_cols.sort()
    
    df_export = df_export[base_cols + dyn_cols] 
    date_cols = ["incorp_date", "hk_incorp_date", "nd2a_eff_date", "nd2a_file_date", "nd4_eff_date", "nd4_file_date", "dissolution_date"]
...
def generate_beautiful_excel(df):
    ...
    headers = ["Client Group", "Company Name EN", "Incorp Place", "Year", "BR Paid By", "BR Status", "BR Deadline", "AR Status", "AR Deadline"]
    ...
    for _, item in df.iterrows():
        row_values = [
            item.get("Client Group", ""), item.get("Company Name EN", ""), item.get("Incorp Place", ""), str(item.get("Year", "")),
            item.get("BR Paid By", ""), item.get("BR Status", ""), item.get("BR Deadline", ""),
            item.get("AR Status", ""), item.get("AR Deadline", "")
        ]
        ...
        for col_idx, val in enumerate(row_values, 1):
            ...
            if col_idx in [1, 3, 4, 5, 7, 9]: cell.alignment = Alignment(horizontal="center", vertical="center")
...
def generate_outstanding_pdf(df):
    ...
    <head>
        ...
        <style>
            ...
            body {{ font-family: 'Noto Sans TC', sans-serif; font-size: 9.5pt; color: #2c3e50; margin: 0; padding: 0; }}
            table {{ width: 100%; border-collapse: collapse; }}
            thead {{ display: table-header-group; }}
            tr {{ page-break-inside: avoid; }}
            th {{ background-color: #1f497d; color: white; padding: 8px 6px; border: 1px solid #d9d9d9; font-size: 10pt; text-align: center; vertical-align: middle; }}
            td {{ padding: 6px; border: 1px solid #d9d9d9; text-align: center; vertical-align: middle; }}
    ...
    <body>
        <table>
            <thead>
                <tr>
                    <td colspan="9" class="report-header-cell">
                ...
                <tr>
                    <th style="width:10%">Client Group</th>
                    <th style="width:23%">Company Name EN</th>
                    <th style="width:11%">Incorp Place</th>
                    <th style="width:6%">Year</th>
                    <th style="width:10%">BR Paid By</th>
                    <th style="width:10%">BR Status</th>
                    <th style="width:10%">BR Deadline</th>
                    <th style="width:10%">AR Status</th>
                    <th style="width:10%">AR Deadline</th>
                </tr>
            </thead>
            <tbody>
    """
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        # Flatten JSON for Dashboard view
        ...
        tab1, tab2 = st.tabs(["📊 All Companies", "🚨 Outstanding List"])
        ...
        with tab1:
            ...
            if 'sel_v146' not in st.session_state: st.session_state.sel_v146 = False
            if t3.button("✅ Select All"): st.session_state.sel_v146 = True; st.rerun()
            if t4.button("🧹 Clear All"): st.session_state.sel_v146 = False; st.rerun()
            
            display_cols = ["name_en"] + [f"{y}_br_paid_by" for y in active_years] + [f"{y}_br_date" for y in active_years] + [f"{y}_ar_date" for y in active_years]
            df_display = df_filtered[display_cols].copy()
            
            df_display.rename(columns={'name_en': 'Company Name EN'}, inplace=True)
            df_display.set_index('Company Name EN', inplace=True)
            df_display.insert(0, "Select", st.session_state.sel_v146)
            
            st.markdown(f"📈 Total: **{len(df_filtered)}** companies in current view.")
            st.info("💡 **Batch Editor:** Edit dynamic yearly records. Save applies instantly to the JSON database field.")
            
            col_cfg = {"Select": st.column_config.CheckboxColumn("Select", default=False)}
            for y in active_years:
                col_cfg[f"{y}_br_paid_by"] = st.column_config.SelectboxColumn(f"✏️ {y} BR Paid By", options=["Firm", "Client"], required=True)
            ...
            if st.button("💾 Save Batch Edits", key="btn_save_grid_v146"):
                try:
                    with engine.begin() as conn:
                        for comp_name, r in edit_df.iterrows():
                            ...
                            for y in active_years:
                                ...
                                br_dt = r.get(f'{y}_br_date')
                                ar_dt = r.get(f'{y}_ar_date')
                                if br_by == 'Client': br_dt = None
                                comp_dict[y_str] = {
                                    "br_paid_by": br_by,
                                    "br_date": br_dt.strftime('%Y-%m-%d') if pd.notna(br_dt) else None,
                                    "ar_date": ar_dt.strftime('%Y-%m-%d') if pd.notna(ar_dt) else None
                                }
                            json_str = json.dumps(comp_dict).replace("'", "''")
                            sql = f"UPDATE companies SET compliance_records = '{json_str}' WHERE name_en = '{comp_name}'"
            ...
            if len(selected) > 0:
                ...
                with act3.popover("🧨 BATCH DELETE"):
                    st.error("🛑 DANGER ZONE"); conf_b = st.text_input("Type DELETE", key="batch_del_v146")
                    if st.button("Confirm Batch Delete", disabled=(conf_b != "DELETE"), key="btn_batch_del_v146"):
        ...
        with tab2:
            ...
            if not df_alerts.empty:
                df_alerts = df_alerts.sort_values(by=['Year', 'Client Group', 'Company Name EN'], na_position='last')
                ...
                if 'sel_alert_v146' not in st.session_state: st.session_state.sel_alert_v146 = False
                if ta3.button("✅ Select All", key="sel_all_alert"): st.session_state.sel_alert_v146 = True; st.rerun()
                if ta4.button("🧹 Clear All", key="clr_all_alert"): st.session_state.sel_alert_v146 = False; st.rerun()
                
                df_alerts_display = df_alerts_filtered.copy()
                df_alerts_display.set_index('Company Name EN', inplace=True)
                df_alerts_display.insert(0, "Select", st.session_state.sel_alert_v146)
                
                st.markdown(f"📈 Total: **{len(df_alerts_display)}** tasks in current view.")
                
                def style_status(val):
                    color = "red" if "Overdue" in str(val) else ("orange" if "Due Soon" in str(val) else "green")
                    return f'color: {color}; font-weight: bold;'
                
                alert_edit = st.data_editor(
                    df_alerts_display.style.map(style_status, subset=['BR Status', 'AR Status']),
                ...
                if export_target is not None:
                    ...
                    c_ex1.download_button("📥 Export Selected PDF", data=pdf_data, file_name="Outstanding_Compliance.pdf", mime="application/pdf")
                    
                    excel_data = generate_beautiful_excel(export_target)
                    c_ex2.download_button("📦 Export Selected Excel", data=excel_data, file_name="Outstanding_Compliance.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                else:
...
# --- 6. Company Register ---
    ...
    d = {'cg': "", 'en': "", 'ch': "", 'place': "", 'p_oth': "", 'idate': None, 'ci': "", 'is_hk_reg': False, 'hk_idate': None, 'hk_ci': "", 'br': "", 'type': "", 'ra': "", 'ca': "", 'rl': "", 'sl': "", 'cl': "", 'n2e': None, 'n2f': None, 'n2d': False, 'n4e': None, 'n4f': None, 'n4d': False, 'dis': None, 'comp_rec_dict': {}}
    target_name = None
    if mode in ["✏️ Edit Existing", "📋 Copy Existing"] and not df_all.empty:
        ...
        if target_name != "":
            ...
            d = {'cg': row.get('client_group', ""), 'en': row.get('name_en', ""), 'ch': row.get('name_ch', ""), 'place': row.get('incorp_place', ""), 'p_oth': row.get('incorp_place_others', ""), 'idate': row.get('incorp_date'), 'ci': row.get('ci_no', ""), 'is_hk_reg': str(row.get('is_hk_registered', "")) == 'True', 'hk_idate': row.get('hk_incorp_date'), 'hk_ci': row.get('hk_ci_no', ""), 'br': row.get('br_no', ""), 'type': row.get('co_type', ""), 'ra': row.get('reg_addr', ""), 'ca': row.get('corres_addr', ""), 'rl': row.get('round_loc', ""), 'sl': row.get('sign_loc', ""), 'cl': row.get('seal_loc', ""), 'n2e': row.get('nd2a_eff_date'), 'n2f': row.get('nd2a_file_date'), 'n2d': str(row.get('nd2a_download', "")) == 'True', 'n4e': row.get('nd4_eff_date'), 'n4f': row.get('nd4_file_date'), 'n4d': str(row.get('nd4_download', "")) == 'True', 'dis': row.get('dissolution_date'), 'comp_rec_dict': comp_rec}
        ...
        if base:
            ...
            for y in active_years:
                with st.expander(f"📌 {y} 年度申報 (Year {y})", expanded=True):
                    ...
                    with col_m1:
                        if y_data.get('br_paid_by') == "Client": st.success(f"### 🟢 BR Fee Deadline ({y}): **{nxt_br.strftime('%Y/%m/%d')}**\n\n✅ **Handled by Client**")
                        else:
                    ...
                    with col_m2:
                        if y > today_cal.year: st.info(f"### 🔵 AR Deadline ({y}): **{nxt_ar.strftime('%Y/%m/%d')}**\n\n🕒 Not yet due")
                        elif ar_days < 0: st.error(f"### 🚨 AR Deadline ({y}): **{nxt_ar.strftime('%Y/%m/%d')}**\n\n⚠️ **Overdue by {abs(ar_days)} days**")
                        elif ar_days <= 30: st.warning(f"### ⏳ AR Deadline ({y}): **{nxt_ar.strftime('%Y/%m/%d')}**\n\n⏰ **Due in {ar_days} days**")
                        else: st.success(f"### 🟢 AR Deadline ({y}): **{nxt_ar.strftime('%Y/%m/%d')}**\n\n✅ **Status: Normal**")
                
                    o1, o2, o3 = st.columns([3, 4, 4])
                    with o1:
                        br_paid_by_idx = 1 if y_data.get('br_paid_by') == "Client" else 0
                        br_by = st.selectbox(f"BR Paid By ({y})", ["Firm", "Client"], index=br_paid_by_idx, key=f"br_by_{y}")
                    with o2: 
                        if br_by == "Client":
                            st.text_input(f"BR Paid Date ({y})", value="N/A (Handled by Client)", disabled=True, key=f"br_dt_dis_{y}")
                            l_br = None
                        else:
                            l_br = st.date_input(f"BR Paid Date ({y})", value=to_date(y_data.get('br_date')), min_value=MIN_DATE, key=f"br_dt_{y}")
                    with o3: 
                        l_ar = st.date_input(f"AR Filed Date ({y})", value=to_date(y_data.get('ar_date')), min_value=MIN_DATE, key=f"ar_dt_{y}")
    ...
    with cc1: n2e = st.date_input("Effective Date (Appt)", value=to_date(d['n2e']), min_value=MIN_DATE, key="n2e_v146")
    with cc2: n2f = st.date_input("Filing Date (ND2A)", value=to_date(d['n2f']), min_value=MIN_DATE, key="n2f_v146")
    with cc3:
        st.info("Statutory Period: 15 days")
        if n2e: n2_deadline = (n2e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n2_deadline}]**") 
    with cc4: n2d = st.checkbox("Downloaded", value=d['n2d'], key="n2d_v146")
    
    st.subheader("📑 Company Secretary Resignation (ND4)")
    cc5, cc6, cc7, cc8 = st.columns([3, 3, 3, 1])
    with cc5: n4e = st.date_input("Effective Date (Resign)", value=to_date(d['n4e']), min_value=MIN_DATE, key="n4e_v146")
    with cc6: n4f = st.date_input("Filing Date (ND4)", value=to_date(d['n4f']), min_value=MIN_DATE, key="n4f_v146")
    with cc7:
        st.info("Statutory Period: 15 days")
        if n4e: n4_deadline = (n4e + timedelta(days=15)); st.markdown(f"**Deadline: :red[{n4_deadline}]**") 
    with cc8: n4d = st.checkbox("Downloaded", value=d['n4d'], key="n4d_v146")
    ...
    row_v146 = {'client_group': client_group, 'name_en': name_en, 'name_ch': name_ch, 'incorp_place': inc_place, 'incorp_place_others': place_others, 'incorp_date': inc_date, 'ci_no': ci_no, 'is_hk_registered': is_hk_reg, 'hk_incorp_date': hk_idate, 'hk_ci_no': hk_ci, 'br_no': br_no, 'co_type': co_type, 'reg_addr': reg_addr, 'corres_addr': corres_addr, 'round_loc': round_l, 'sign_loc': sign_l, 'seal_loc': common_l, 'nd2a_eff_date': n2e, 'nd2a_file_date': n2f, 'nd2a_download': n2d, 'nd4_eff_date': n4e, 'nd4_file_date': n4f, 'nd4_download': n4d, 'dissolution_date': dis_date, 'compliance_records': json.dumps(updated_comp_json)}
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        if st.button("💾 Save To Cloud", key="btn_save_v146"):
            if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
            else:
                try:
                    pd.DataFrame([row_v146]).to_sql('companies', engine, if_exists='append', index=False)
        ...
        with u_col.popover("🆙 Update"):
            if st.button("Confirm Update", key="btn_update_v146"):
                if missing: st.error(f"❌ Missing mandatory fields: {', '.join(missing)}")
                else:
                    try:
        ...
        with d_col.popover("🚨 DELETE"):
            st.error(f"Delete {target_name}?"); conf_s = st.text_input("Type DELETE", key="single_del_v146")
            if st.button("Confirm Delete Company", disabled=(conf_s != "DELETE"), key="btn_del_single_v146"):
...
# --- 7. Group Management ---
    ...
    new_g = st.text_input("New Group Name", key="new_group_input_v146")
    if st.button("Add Group", key="btn_add_group_v146"): pd.DataFrame([{'group_name': new_g}]).to_sql('client_groups', engine, if_exists='append', index=False); st.rerun()
    st.write("---")
    g_df = pd.read_sql("SELECT * FROM client_groups", engine)
    if not g_df.empty:
        g_df = g_df.sort_values(by=['group_name'], na_position='last')
        target = st.selectbox("Select Group", g_df['group_name'].tolist(), key="select_group_manage_v146")
        c1, c2 = st.columns(2)
        with c1.popover("✏️ Rename Group"):
            ren = st.text_input("New Name:", key="rename_input_v146")
            conf_r = st.text_input("Type RENAME", key="rename_confirm_text_v146")
            if st.button("Confirm Rename", disabled=(conf_r != "RENAME"), key="btn_group_rename_v146"):
        ...
        with c2.popover("🗑️ Delete Group"):
            if st.button("Confirm Delete Group", key="btn_group_delete_v146"): 
...
# --- 8. Data Exchange ---
    ...
    # Template Generation with Dynamic Years
    ...
    c2.download_button(label="📦 Export All", data=buf_e.getvalue(), file_name="Backup.xlsx", key="btn_export_all_v146")
    st.write("---")
    
    up = st.file_uploader("Upload XLSX to Review Changes", type=["xlsx"], key="file_uploader_v146")
    if up:
        try:
            ...
            if validation_errors:
                ...
                diff_list = []
                # Simple Sync Check (Ignoring JSON internals for diff display simplicity)
                ...
                if diff_list or True: # Force sync allow for JSON fields
                    st.subheader("🔍 Review Changes (Base Info)")
                    if diff_list: st.table(pd.DataFrame(diff_list))
                    else: st.info("Base info matches. Click sync to update compliance dates.")
                    
                    if st.button("🚀 Confirm & Apply Changes", key="btn_final_sync_v146"):
                        for _, row_new in up_df.iterrows():
                            # Pack dynamic columns to JSON
                            comp_dict = {}
                            for y in active_years:
                                y_str = str(y)
                                br_by = str(row_new.get(f'{y} BR Paid By', 'Firm')).strip()
                                if br_by == '': br_by = 'Firm'
                                br_dt = row_new.get(f'{y} BR Date')
                                ar_dt = row_new.get(f'{y} AR Date')
                                comp_dict[y_str] = {
                                    "br_paid_by": br_by,
                                    "br_date": str(br_dt) if br_dt and pd.notna(br_dt) and str(br_dt) != 'NaT' else None,
                                    "ar_date": str(ar_dt) if ar_dt and pd.notna(ar_dt) and str(ar_dt) != 'NaT' else None
                                }
                            row_new['compliance_records'] = json.dumps(comp_dict)
                        
                        # Remove dynamic columns before merging
