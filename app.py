...
def generate_custom_pdf(selected_df):
    ...
    <head>
        ...
        <style>
            ...
            .company-card {{ page-break-after: always; border: 1px solid #dcdde1; border-radius: 8px; padding: 20px; margin-bottom: 20px; background-color: #fbfbfb; }}
            ...
            th {{ text-align: left; width: 45%; color: #7f8c8d; padding: 5px 0; font-size: 10pt; }}
            td {{ padding: 5px 0; color: #2c3e50; font-size: 10pt; }}
            .section-title {{ background: #f1f4f6; padding: 5px 10px; font-weight: bold; margin-top: 15px; border-left: 4px solid #3498db; }}
        </style>
    </head>
    <body>
        <div class="header"><h1>Corporate Portfolio Report</h1><p>Generated on: {now}</p></div>
    """
...
if choice == "📊 Dashboard":
    ...
    if not df_raw.empty:
        ...
        edited_df = st.data_editor(df_display, column_config={"Select": st.column_config.CheckboxColumn("Select", default=False)}, hide_index=True, use_container_width=True, key="dash_v58")
        ...
        if len(selected_rows) > 0:
            ...
            with act2.popover("🧨 BATCH DELETE"):
                ...
                conf_batch = st.text_input("Type **DELETE** to confirm", key="batch_del_v58")
...
# --- 5. Company Register (UI 鎖定截圖 image_2c2193.png + 紅框 + 獨立刪除) ---
    ...
    cc3.info("Statutory Period: 15 days"); n2d = cc4.checkbox("Downloaded", value=d['n2d'], key="n2d_v58")
    ...
    if mode in ["🆕 Add New", "📋 Copy Existing"]:
        ...
        with btn_d.popover("🚨 DELETE Record"):
            ...
            conf_s = st.text_input("Type **DELETE** to confirm", key="single_del_v58")
...
# --- 6. Group Management (強力提醒鎖定) ---
    ...
    if not g_df.empty:
        ...
        with c2.popover("🗑️ Delete Group"):
            ...
            conf_g = st.text_input("Type **DELETE** to confirm", key="grp_del_v58")
...
# --- 7. Data Exchange (恢復 Feedback 與 雙欄佈局) ---
    ...
    col_ex2.download_button(label="📦 Export Full Backup", data=buf_exp.getvalue(), file_name="Backup.xlsx")
    st.write("---")
